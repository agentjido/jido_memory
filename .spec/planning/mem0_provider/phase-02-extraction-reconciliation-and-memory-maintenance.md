...
[ ] 2 Phase 2 - Extraction, Reconciliation, and Memory Maintenance
  ...
  [ ] 2.1 Section - Extraction Pipeline and Candidate Memory Generation
    ...
    [ ] 2.1.1 Task - Implement Mem0 extraction ingestion
      Introduce provider-defined extraction that converts interaction inputs into candidate memory facts.

      [ ] 2.1.1.1 Subtask - Implement `Jido.Memory.Capability.Ingestion` on the Mem0 provider.
      [ ] 2.1.1.2 Subtask - Accept conversational or interaction-oriented payloads that can generate candidate memories from message pairs or interaction batches.
      [ ] 2.1.1.3 Subtask - Return deterministic ingestion summaries describing extracted candidates, skipped candidates, and created or updated memory ids.

    [ ] 2.1.2 Task - Add summary and recency context handling
      Support the contextual inputs Mem0-style extraction needs without widening the shared plugin or runtime facade.

      [ ] 2.1.2.1 Subtask - Define provider config for recent-message windows and optional conversation-summary context.
      [ ] 2.1.2.2 Subtask - Keep summary generation provider-owned rather than a new shared runtime responsibility.
      [ ] 2.1.2.3 Subtask - Surface extraction-context settings through `info/2` and provider metadata.
