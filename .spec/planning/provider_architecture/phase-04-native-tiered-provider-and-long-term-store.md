...
- [x] Create `Jido.Memory.Provider.Tiered` implementing `Jido.Memory.Provider`.
- [x] Add a stable built-in provider selection path for Tiered through `Jido.Memory.ProviderRef` and plugin configuration.
- [x] Return Tiered-specific provider metadata and structured capability data from `init/1` and `capabilities/1`.
...
- [x] Define provider config for short, mid, and long storage plus lifecycle policy options.
- [x] Provide sensible defaults so Tiered works out of the box with ETS-backed stores in development.
- [x] Validate tier, promotion, and consolidation settings through the provider config boundary.
...
- [x] Introduce a long-term persistence behaviour or equivalent abstraction suitable for the Tiered provider.
- [x] Provide a default ETS-backed long-term implementation for the built-in path.
- [x] Implement promotion and consolidation flows across short, mid, and long tiers.
...
- [x] Implement retrieval across one or more tiers through the provider `retrieve/3` callback.
- [x] Add provider-managed metadata needed to explain tier placement, promotion decisions, and consolidation outcomes.
- [x] Expose lifecycle and retrieval capability metadata in a way the common runtime can inspect.
...
- [x] Route Tiered provider selection cleanly through existing provider normalization.
- [x] Preserve the current compatibility behavior for Basic when no provider is specified.
- [x] Keep unsupported-capability behavior deterministic for providers that are still core-only.
...
- [x] Make `Jido.Memory.Plugin` mount cleanly with the built-in Tiered provider.
- [x] Keep `Remember`, `Retrieve`, `Recall`, and `Forget` action behavior stable across Basic and Tiered.
- [x] Preserve checkpoint, restore, and auto-capture behavior through the Tiered provider path.
...
- [x] Verify `Jido.Memory.Provider.Tiered` passes the shared provider contract suite.
- [x] Verify Tiered reports lifecycle and retrieval capability metadata through the standard capability map.
- [x] Verify invalid Tiered configuration fails deterministically at the provider boundary.

### Task 4.4.2 - Tiered runtime and plugin scenarios
Description: prove the common runtime and plugin can use Tiered end to end.
- [x] Verify the same agent fixture can switch between Basic and Tiered without changing common plugin code.
- [x] Verify Tiered promotion, consolidation, and long-term persistence behavior through the provider-aware runtime surface.
- [x] Verify `Retrieve` and `Recall` remain compatible against Tiered-backed records.
