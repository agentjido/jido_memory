defmodule Jido.Memory.RuntimeNormalizationTest do
  ...
  defmodule ShapeProvider do
    ...
    def capabilities(_opts), do: {:ok, [:retrieve, :ingest]}
  ...
  test "runtime normalizes provider list and map shapes" do
    assert {:ok, %CapabilitySet{capabilities: [:retrieve, :ingest]}} =
  ...
  test "runtime surfaces malformed provider payloads as canonical errors" do
    ...
    assert {:error, {:invalid_ingest_result, :bad}} = Runtime.ingest(%{}, %{records: []}, provider: MalformedProvider)

    assert {:error, {:invalid_explanation, :bad}} =
             Runtime.explain_retrieval(%{}, %{namespace: "agent:test"}, provider: MalformedProvider)

    assert {:error, {:invalid_consolidation_result, :bad}} = Runtime.consolidate(%{}, provider: MalformedProvider)
  end
